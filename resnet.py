...
def ResNet(input_shape=None, classes=10, block='bottleneck', residual_unit='v2',
		   ...
		   input_tensor=None, dropout=0.2, transition_dilation_rate=(1, 1),
	...
	#to be on safer side min_size can be taken as input_shape[1]
	# Determine proper input shape
	input_shape = _obtain_input_shape(input_shape,
									  default_size=32,
									  min_size=int(input_shape[1]),
...
class ResnetBuilder(object):
	...
	def build(input_shape, num_outputs, block_fn, repetitions):
		...
		if block_fn == 'basic':
			...
			block = bottleneck
		elif isinstance(block_fn, six.string_types):
			block = _string_to_function(block)
		else:
			block = block_fn
